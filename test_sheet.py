...
def update_row(sheet, row_number, data):
    sheet.update(f"A{row_number}:E{row_number}", [data])
...
def main():
    ...
    st.subheader("Edit Record (by Row Number)")
    with st.form("edit_form"):
        row_number = st.number_input("Row Number to Edit (starts at 2)", min_value=2, step=1)
        patient_name = st.text_input("New Patient Name")
        service = st.text_input("New Service")
        amount_paid = st.number_input("New Amount Paid", min_value=0.0, step=0.01, key="edit_amount")
    ...
    st.subheader("Delete Record (by Row Number)")
    with st.form("delete_form"):
        del_row_number = st.number_input("Row Number to Delete (starts at 2)", min_value=2, step=1, key="delete")
